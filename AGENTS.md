...
- **Recipe review forbids inline styles** for: `display`, `justify-content`, `padding`, `margin`, `text-align`, `font-size`, `object-fit`, `background-color`, `color`, `border-radius`. Use framework utilities instead: `flex flex--col` (the combo — also sidesteps the lg display:block bug), `stretch` (a flex-grow spacer, replaces space-between constructs), `w--full`/`h--full`, `text--right`/`text--center`, `block`, `pr--5`, `lg:mt--{5|10|16|20|24|40}`. Still allowed inline: `flex` basis values, `min-width`/`min-height`, `max-height`, `white-space`, `align-items`, fixed px `width`/`height`.
